...
# Source user's original .zshrc nếu có
if [[ -f "$HOME/.zshrc" ]]; then
  source "$HOME/.zshrc"
fi

# Hiển thị prefix màu đỏ
red=$'\033[1;31m'
reset=$'\033[0m'
PROMPT="[$red$SCTX_PROFILE$reset] %%n@%%m %%1~ %%# "
setopt PROMPT_SUBST

# Load profile thực tế của sctx
if [[ -f "%s" ]]; then
  source "%s"
fi

# Hook exit hiển thị log
function exit() {
  echo "👋 Leaving profile: $SCTX_PROFILE"
  builtin exit
}
`, name, profilePath, profilePath)
		...
		// ⚙️ Thay thế process hiện tại bằng zsh
